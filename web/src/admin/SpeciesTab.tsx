...
export function SpeciesTab(props: AdminTabProps) {
  ...
  async function save() {
    ...
    try {
      ...
      parseSpeciesReceipt(raw, mode === "create" ? { code: draft.code } : { id: selected?.id });
  ...
  return <div className="admin-stack">{notice ? <div role={notice.kind === "error" ? "alert" : "status"} className={`notice notice--${notice.kind}`}>{notice.text}</div> : null}
    ...
    <QueryBoundary query={query}>{(data) => data.items.length ? <><div className="admin-table-wrap"><table><thead><tr><th>代码</th><th>中文名</th><th>英文名</th><th>学名</th><th>排序</th><th>候选数</th><th>状态</th><th>操作</th></tr></thead><tbody>{data.items.map((item) => <tr key={item.id}><th>{item.code}</th><td>{item.name_zh}</td><td>{item.name_en}</td><td><em>{item.scientific_name}</em></td><td>{item.sort_order}</td><td>{item.candidate_count}</td><td>{item.active ? "启用" : "停用"}</td><td><button type="button" className="secondary-button" aria-label={`编辑 ${item.code}`} onClick={() => startEdit(item)}>编辑</button></td></tr>)}</tbody></table></div><PageControls offset={offset} total={data.total} limit={LIMIT} onChange={setOffset} /></> : <p>没有鱼种。</p>}</QueryBoundary>
    {mode ? <section className="admin-card"><h3>{mode === "create" ? "新增鱼种" : `编辑 ${selected?.code}`}</h3>
