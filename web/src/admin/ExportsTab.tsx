...
import { QueryBoundary, adminMutation, mutationMessage, useAdminQuery, type AdminTabProps } from "./common";
import { parseExportBatches, parseExportCreate, parsePendingCounts, parseReceiptFile, parseReceiptResponse, type ExportBatch } from "./types";
...
export function ExportsTab(props: AdminTabProps) {
  ...
  const batches = useAdminQuery("/admin/exports", parseExportBatches, props.retryBootstrap);
  ...
  async function create() {
    if (pending) return; setPending(true); setNotice(null);
    ...
    catch (error) { setNotice({ kind: "error", text: error instanceof ApiError && error.status === 409 ? "存在范围重叠或状态冲突的批次，列表已刷新。" : mutationMessage(error) }); if (error instanceof ApiError && error.status === 409) { counts.reload(); batches.reload(); } }
  ...
  async function receiptFile(batch: ExportBatch, file: File | null) {
    ...
    setPending(true);
    try {
      const parsedJson: unknown = JSON.parse(await file.text()); const upload = parseReceiptFile(parsedJson, batch.id); const candidates = new Set(upload.items.map((item) => item.candidate_id));
      const raw = await adminMutation<unknown>(`/admin/exports/${batch.id}/receipt-file`, { method: "POST", body: upload, csrfToken: props.csrfToken }, props.retryBootstrap); const result = parseReceiptResponse(raw, batch.id, candidates); setNotice({ kind: "success", text: `回执已处理：接受 ${result.accepted}，待处理 ${result.pending}。` }); batches.reload(); counts.reload();
