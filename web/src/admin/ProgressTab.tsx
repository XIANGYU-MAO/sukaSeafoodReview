...
export function ProgressTab(props: AdminTabProps) {
  ...
  async function submit() {
    ...
    try {
      ...
      parseCandidateReceipt(raw, { id: candidate.id, previousVersion: candidate.version, currentReviewerId: action.kind === "release" ? null : target });
  ...
  const sources = [...new Set((current.data?.items ?? []).map((item) => item.candidate.source_dataset))];
  return <div className="admin-stack">
    ...
    <section className="admin-card"><h3>当前打开的图片</h3>
      <form className="admin-filters" onSubmit={(event) => { event.preventDefault(); setApplied(filters); }}>
      ...
      <QueryBoundary query={current}>{(data) => data.items.length ? <div className="admin-table-wrap"><table><thead><tr><th>成员</th><th>候选 ID</th><th>来源记录</th><th>鱼种</th><th>打开时间</th><th>操作</th></tr></thead><tbody>{data.items.map((row) => <tr key={row.candidate.id}><td>{row.reviewer.display_name}</td><td className="mono">{row.candidate.id}</td><td>{sourceLabel(row.candidate.source_dataset)} · {row.candidate.source_record_id}</td><td>{row.species.code} · {row.species.name_zh}</td><td>{new Date(row.current_started_at).toLocaleString("zh-CN")}</td><td><div className="inline-actions"><button className="danger-button" type="button" onClick={() => start("release", row)}>释放</button><button className="secondary-button" type="button" onClick={() => start("transfer", row)}>转交</button></div></td></tr>)}</tbody></table></div> : <p>当前没有成员打开图片。</p>}</QueryBoundary>
    </section>
    {action ? <section className="admin-card admin-confirm" aria-label={action.kind === "release" ? "确认释放当前图片" : "确认转交当前图片"}>
