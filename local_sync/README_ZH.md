...
这个工具由管理员在自己的 Windows 电脑上运行。它从管理后台导出的增量 CSV 读取服务器指定的操作，只让本机直接向获准的外部来源站点获取新审核通过的原图。图片不会经过中国服务器，也不会存放在中国服务器；服务器只提供受限 CSV 并接收受限回执。每批最多 10,000 行，精确 17 列 CSV、在线回执和离线回执各自最多 20 MiB。
...
- 只有新审核通过的 ADD 原图由管理员的电脑从外部来源直接下载。
...
清单和每次 HTTP 重定向只允许获准的 HTTPS 精确主机或域名后缀；localhost、IPv4/IPv6 字面量、含凭据 URL、非 443 端口和未批准主机会在发出请求前拒绝。服务器导出的 `image_origin_allowlist` 列会携带当前批次实际使用且已获批准的精确主机；管理员在后台批准新来源后，本地工具会直接读取，不需要升级程序。默认目录和可选的 `SUKASEAFOOD_IMAGE_ORIGIN_ALLOWLIST` 仍可补充部署级来源；服务器侧对应变量是 `IMAGE_ORIGIN_ALLOWLIST`。不要把宽泛公共后缀加入列表，也不要仅靠 DNS 解析判断公开性：批准的主机名边界可避免解析与连接之间的地址变化，使用代理时连接地址由受信代理负责。
...
Windows 构建只保存在开发电脑的 `local_sync/dist/`，不由线上网页分发；运行时请保留完整目录，不要只复制 exe。
