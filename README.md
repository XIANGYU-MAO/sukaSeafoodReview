...
The CLI dry-run writes no candidates and creates no committable preview token. The CLI and production helper still require zero blocking errors and `can_commit=true`, then revalidate transactionally with `--commit` and print `total`, `inserted`, `skipped_exact`, `skipped_url_duplicates`, and `skipped_blocking`. The Chinese admin accepts a dropped CSV: the same canonical original URL for the same species is skipped automatically, while assigning one image to different species blocks. Valid unknown HTTPS image hosts are grouped by exact hostname; Mao may approve an observed host once from the current preview and the retained file is previewed again automatically. Localhost, literal IPs, credential-bearing URLs, and other intrinsically unsafe forms can never be approved. Mao may also explicitly confirm importing only the valid rows while skipping remaining blockers. A Web preview token remains only in current-page memory and expires; a new file, terminal conflict, or successful commit invalidates it.
...
The server uses one envelope for incremental batches: at most 10,000 rows per batch, at most 20 MiB after serializing the exact 17-column CSV, and at most 20 MiB for an online or offline receipt upload. More than 10,000 eligible operations are split into later, non-overlapping batches; a single row that breaches the byte limit fails before any batch is persisted. ADD, REMOVE, and MOVE rows come from one coherent PostgreSQL snapshot and carry the server-selected exact relative path, a monotonic **candidate synchronization generation**, and the approved image hosts actually used by the batch. The fixed wire column remains named `review_version` for compatibility; its value is the candidate's synchronization generation, not a reviewer's edit count. CSV download is an authenticated same-origin, `no-store` attachment; receipt submission is a bounded `application/json` POST.
...
The independent `local_sync` package, CLI/Tkinter UI, and Windows executable are implemented. Mao's computer contacts each approved `original_url` directly, validates every redirect, verifies image content and hashes, uses `.part` plus atomic rename for idempotent resume, and moves REMOVE targets into recoverable `_removed` paths. The export CSV's `image_origin_allowlist` column carries the exact approved hosts actually used by that batch, so approving a new source does not require a local-tool upgrade or manual environment change. `IMAGE_ORIGIN_ALLOWLIST` and `SUKASEAFOOD_IMAGE_ORIGIN_ALLOWLIST` remain available for deployment-level extensions. Localhost, IP literals, and unapproved sources are rejected. A configured proxy is trusted only to connect to an already approved hostname; the downloader sends no cookies or credentials to image sources. The China server never issues image HEAD/GET requests and has no image cache or proxy.
