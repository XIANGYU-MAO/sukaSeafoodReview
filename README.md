...
This repository contains the collaborative review core for SukaSeafood fish-image candidates. Six fixed accounts—Hassan, Mao, Xinhui, Wahid, Sharmaa, and Yiming—share one review pool; Mao is the sole administrator. There are no per-person quotas, and a candidate is never assigned to someone who has already reviewed it. Each KEEP, REJECT, or UNSURE choice is written immediately, and the next candidate is requested only after the database confirms the result.

Reviewers can view and edit only their own current history, while everyone can see aggregate progress. Mao has a Chinese-only, seven-tab administration area for progress, candidates, species, review history, CSV imports, training-sync batches, and accounts. The server stores structured candidate metadata, external URLs, review results, CSV files, and small receipts. It never stores, caches, proxies, or downloads original image bytes. Incremental downloading of approved originals is a later Windows local-sync stage in which Mao's computer contacts external sources directly.
...
The system does not provide `/project` or `/project/*`, nor does it expose an image-upload, original-image proxy, or original-image download API. Removing the legacy `/project` routes while protecting other YGF pages belongs to the later production plan and is not performed by local setup.
...
- SQLite is supported for local development and tests only.
...
The CLI dry-run writes no candidates and creates no committable preview token. After reviewing the report, Mao chooses the same CSV on the Chinese “导入” tab, runs preview, and explicitly confirms commit. The preview token remains only in the current page's memory and expires; a new file, terminal conflict, or successful commit invalidates the old token.
...
The current core creates small incremental CSV batches and accepts receipts; it does not include the Windows downloader. CSV rows express ADD, REMOVE, or MOVE from current database state and carry the exact server-selected target relative path. The download is an authenticated same-origin GET, and the bounded receipt-file endpoint is an `application/json` POST.

The later `docs/superpowers/plans/2026-08-26-local-training-sync.md` stage will add an independent `local_sync` package, CLI/Tkinter UI, and Windows executable. That tool is planned to contact each `original_url` directly from Mao's computer, validate images, calculate hashes, use `.part` plus atomic rename, resume idempotently, and move REMOVE targets into a recoverable `_removed` path. Keeping originals off the China server avoids server bandwidth, upstream rate-limit, and licence-boundary expansion while preserving local ownership of training data.

Until the downloader stage is complete, do not describe an exported CSV as a synchronized local training set and do not fabricate successful receipts manually.
...
Production assets must remain under `/sukaseafood/review/assets/`. These commands verify code. Public routes, Caddy, backups, six-account browser acceptance, and YGF regressions belong to the later deployment plan.
...
- Later Windows local-sync plan: `docs/superpowers/plans/2026-08-26-local-training-sync.md` (not implemented yet)
- Later production and YGF routing plan: `docs/superpowers/plans/2026-08-26-production-deployment.md` (not executed yet)

The production stage still requires PostgreSQL, containers, backups, Caddy, `/project` 404 checks, public-route acceptance, and rollback verification. This document contains no real server, SSH, database, production-password, or secret values.
