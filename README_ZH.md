...
成员只能查看和修改自己的当前审核历史；顶部“团队记录”页面向所有成员展示汇总进度，不展示他人的审核详情。登录页和登录后界面都可切换中文/English，中文界面的密码入口只显示“修改密码”。每次成功登录后首次进入审核页会显示一次双语审核标准，确认后刷新或切换页面不会重复。Mao 使用标题为“管理后台”的固定中文七标签后台管理进度、候选图片、动态鱼种、审核历史、CSV 导入、训练集同步批次和账号。服务器只保存结构化候选信息、外部 URL、审核结果、受限 CSV 与回执，不保存、缓存、代理或下载原图字节。Windows 本地同步工具已经实现，由 Mao 的电脑直接访问获准的外部来源；详见 [`local_sync/README_ZH.md`](local_sync/README_ZH.md)。
...
- 审核提交带 CSRF 和 Idempotency-Key；API 在数据库事务确认后返回回执，网页验证回执再取下一张。独立“团队记录”页面在打开时读取最新汇总。
...
登录页用六个椭圆按钮选择固定姓名，并可直接切换中文/English；选择、密码输入和可见错误在切换时保留，登录成功后继续使用所选语言。显式成功登录会为该成员开启一次新的审核提示周期；第一次进入审核页确认“审核标准”后，同一登录会话内刷新、查看历史或团队记录再返回都不会重复显示。恢复仍有效的 Cookie 会话不会重置该标记。
...
1. 用椭圆按钮选择固定姓名并登录；登录页可切换中文/English，首次登录先改密码。
2. 每次成功登录后首次进入审核页会显示一次“审核标准”：鱼种可确认、鱼体或关键特征清楚时可通过；干净博物馆标本、冰上或市场中的真实鱼以及主体明确的多条同种鱼都可通过。鱼种无法确认、鱼群太小或严重重叠、多种鱼混杂、严重模糊遮挡，以及切块、烹饪、插画或重复图片不通过。确认后同一登录内不重复。
3. 首页从共享池恢复或取得一张尚未由该成员审核的候选图。
4. 图片加载期间显示转圈；失败会进入有限错误状态，可重试或选择“图片链接不可用”。
5. 查看中英文鱼种名、学名、来源、来源记录、许可与安全外链。应用只把 URL 交给浏览器，不抓取图片。
6. 选择 KEEP、REJECT 或 UNSURE。REJECT 必须选择椭圆形原因；“其他”必须输入说明。K/R/U 快捷键不会劫持输入控件。
7. 网页立即提交并等待数据库回执；回执身份、内容和版本验证成功后才取下一张，没有额外保存按钮。
8. 顶部“历史记录”只请求当前成员自己的记录；旁边的“团队记录”只请求全组聚合。历史 URL 不带 reviewer 查询参数，只有当前版本可编辑；旧版本只读，409 冲突不会静默覆盖。
...
6. 训练集同步：创建增量批次和任务 CSV，在开发电脑运行本地 Windows 原图同步工具；自动回执失败时可拖入 JSON。待处理批次可“废弃并重新创建”，只重排未完成项目；历史卡片可以移除，但成功记录会继续用于防止重复下载。线上网页不分发同步工具。
